...
1. Поле `GO`
2. Поле `Собственность` 
3. Поле `Налог`
4. Поле `Шанс/Общественная Казна`
5. Поле `Бесплатная стоянка`
6. Поле `Направиться в тюрьму`
7. Поле `Посетить тюрьму`
...
Если игрок попал на поле `Собственность` и оно не принадлежит другому игроку и не заложено в банк, то происходит *процесс покупки* (см. раздел 2.3.1. "Покупка и продажа"). Если игрок желает купить собственность, то он платит в банк сумму, **указанную на поле** игровой карты как `начальная стоимость` в банк, затем банкир быдает ему *карточку собственности*. После этого игрок считается *обладателем собственности*.
...
>Таким образом, после того как игрок попал на собственность, которая **никому не принадлежит**, эта собственность **попадает во владение** какому-нибудь игроку или **возвращается** в банк, если ее не смогли продать на аукционе.
...
Все `собственности` обладают характеристиками:
| Название характеристики | Тип значения    |
| ----------------------- | --------------- |
| `Название`              | строка символов |
| `Начальная стоимость`   | число           |
| `Залоговая стоимость`   | число           |

Помимо общих характеристик, у разных типов `собственностей` есть свои специфические характеристики которые будут описаны далее.
...
Рассмотрим характеристику `цвет`. Одинаковым цветом обозначается группа улиц имеющих одинаковое значение характеристики `цвет`. Цвет важен, когда игрок хочет купить *дом* или *отель* (см. разделы 2.3.1.2 "Дома", 2.3.1.3 "Отели"). *Группа улиц* одного цвета может состоять из 3 или 2 отдельных улиц.

> Если у игрока во владении находится **вся** группа улиц одного цвета, то *владелец собственности* может попросить удвоенную сумму от начальной суммы ренты (характеристика `Начальная сумма ренты`). В иных случаях работают стандартные **правила сбора ренты** (см. раздел 2.3.3. "Рента")
...
> Для полей этого типа **не указана** конкретная стоимость ренты, она рассчитывается согласно тому, **сколько очков выпало** на костях у игрока на момент попадания в это поле (см. раздел 2.3.3. "Рента")
...
Игрок, который попал на это поле имеет право не выполнять никаких действий в текущем ходу.
